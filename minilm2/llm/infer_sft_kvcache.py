...
def build_context(history: list[tuple[str, str]], tokenizer: Tokenizer,
    ...
    text = history[-1]
    ids += human_prefix_ids + tokenizer.encode(text[0]).ids + separator_ids
    ids += ai_prefix_ids + tokenizer.encode(text[1]).ids
...
def async_input():  # 用于修复调试时concole无法单独开进程input的问题
    def worker():
        tmp = sys.stdin.read().strip()
...
if __name__ == '__main__':
    ...
    config_path = os.path.join(os.path.dirname(os.path.dirname(current_dir)), "models/pretrain/pretrain_ngpt.json")
    ...
    # model.compile(fullgraph=True) # 本地console请注释掉
    model.eval()
    ...
    while True:
        text, history = get_input(history)
        # 构建输入
        ...
        response = ""
        n_blankline = 0
        position_idx = input_ids.numel()
        with torch.no_grad():
            while True:
                try:
                    output = model(input_ids, position_idx)
